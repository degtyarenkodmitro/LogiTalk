...
class App(CTk):
    def __init__(self):
        ...
        self.title("sHARIKI upali")
        """MENU"""
        self.menu_frame = CTkFrame(self, fg_color="#86eb34", width=30, height=400)
        self.menu_frame.pack_propagate(False)
        self.menu_frame.place(x=0, y=0)
        #self.menu_lbl = CTkLabel(self, text="введіть ім'я", font=("Helvetica", 20, "bold"))
    ...
    def toggle_menu(self):
        if self.is_menu:
            ...
            self.btn_save = CTkButton(self.menu_frame, text="Зберегти", command=self.save_name)
    ...
    def adaptive_ui(self):
        ...
        self.msg_text_entry.place(x=self.menu_frame.winfo_width() + 5, y=self.winfo_height() - 55)
    ...
    def add_recv_msg(self, text):
        msg_frame = CTkFrame(self.scroll_frame, fg_color="#86eb34")
    ...
    def send_msgs(self):
        ...
        if text:
            ...
            try:
                self.sock.send(f"{self.name}: {text}".encode())
            except:
                pass
